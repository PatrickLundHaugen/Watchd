                ...
                {series.map((movie) => (
                    <div key={movie.id} className="flex flex-col bg-card rounded-xl border border-input shadow-sm overflow-hidden">
                        ...
                        <div className="flex flex-col justify-between grow gap-2 p-4">
                            <div>
                                <Link href={`/movie/${movie.id}`} className="leading-none font-semibold tracking-tight cursor-pointer hover:underline">{movie.name}</Link>
                                <p className="text-sm text-muted-foreground">{movie.first_air_date?.slice(0, 4)}</p>
                            </div>
